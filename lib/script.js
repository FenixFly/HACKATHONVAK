...
async function setupJournalDemo(setupJournalDemo) {
 ...
 // create JOwners
 var journalOwners = [
  factory.newResource(NS, 'JournalOwner', 'MSU'),
  factory.newResource(NS, 'JournalOwner', 'UNN'),
  factory.newResource(NS, 'JournalOwner', 'NTSU'),
];

  // fill animals with basic info
  journalOwners[0].firstName = 'Ivan';
  journalOwners[0].lastName = 'Ivanov';
  journalOwners[0].inn = '1234567890';
  journalOwners[0].email = 'admin@msu.ru';

  journalOwners[1].firstName = 'Petr';
  journalOwners[1].lastName = 'Petrov';
  journalOwners[1].inn = '1234567891';
  journalOwners[1].email = 'admin@unn.ru';

  journalOwners[2].firstName = 'Sergey';
  journalOwners[2].lastName = 'Sergeev';
  journalOwners[2].inn = '1234567892';
  journalOwners[2].email = 'admin@ntsu.ru';
  ...
  await journalOwnerRegistry.addAll(journalOwners);
   ...
   // create Journals
   var journals = [
    factory.newResource(NS, 'Journal', '0001-0001'),
    factory.newResource(NS, 'Journal', '0001-0002'),
    factory.newResource(NS, 'Journal', '0001-0003'),
  ];
    ...
    journals[0].title = 'Mathematical modelling'
    ...
    journals[1].title = 'Computer graphics'
    ...
    journals[2].title = 'Geometric modelling'
...
/**
 * @param {org.sample.ProposeVAK} tx
 * @transaction
 */
async function proposeVAK(tx) {
  
  tx.journal.isVAK = tx.newStatus;

  // get journals registry
  const ar = await getAssetRegistry('org.sample.Journal');
  // update the journal in the registry
  await ar.update(tx.journal);
}